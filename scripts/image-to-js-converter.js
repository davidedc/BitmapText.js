...
function generateJSContent(idString, base64Data, imageType, originalFilename, positioningData) {
    ...
    // PHASE 1: Positioning data is optional (null for new format, present for legacy)
    if (positioningData) {
        // Legacy format: Atlas with embedded positioning data
        return `// Atlas data for ${originalFilename} (LEGACY FORMAT - includes positioning)
...
`;
        ...
        // Phase 1 format: Atlas only (NO positioning data - will be reconstructed at runtime)
        return `// Atlas data for ${originalFilename} (PHASE 1 FORMAT - no positioning data)
// Generated by image-to-js-converter.js
// MIME type: ${mimeType}
// Encoding: base64
...
if (processPNG) {
    ...
    pngFiles.forEach(pngFile => {
        ...
        // PHASE 1: Positioning data NO LONGER exists - will be reconstructed at runtime
        // Try to read positioning data for backward compatibility, but don't require it
        const positioningData = tryReadPositioningData(targetDir, idString);
        if (positioningData) {
            log(`Found positioning data for ${idString} (legacy format)`);
        } else {
            log(`No positioning data for ${idString} (Phase 1 format - will be reconstructed at runtime)`);
        }

        const jsContent = generateJSContent(idString, base64Data, 'png', pngFile, positioningData);

        fs.writeFileSync(jsFilePath, jsContent);
        const formatMsg = positioningData ? '(with positioning data - legacy)' : '(Phase 1 - no positioning)';
        log(`Processed PNG: ${pngFile} -> ${jsFileName} ${formatMsg}`);
...
if (processQOI) {
    ...
    qoiFiles.forEach(qoiFile => {
        ...
        fs.writeFileSync(jsFilePath, jsContent);
        const formatMsg = positioningData ? '(with positioning data - legacy)' : '(Phase 1 - no positioning)';
        log(`Processed QOI: ${qoiFile} -> ${jsFileName} ${formatMsg}`);
