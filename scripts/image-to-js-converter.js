...
function generateJSContent(idString, base64Data, imageType, originalFilename, positioningData) {
    ...
    // Positioning data is optional (null for new format, present for legacy)
    if (positioningData) {
...
`;
        ...
        // New format: Atlas only (NO positioning data - will be reconstructed at runtime)
        return `// Atlas data for ${originalFilename} (no positioning data)
// Generated by image-to-js-converter.js
// MIME type: ${mimeType}
// Encoding: base64
...
if (processPNG) {
    ...
    pngFiles.forEach(pngFile => {
        ...
        // Positioning data NO LONGER exists - will be reconstructed at runtime
        ...
        if (positioningData) {
            ...
            log(`No positioning data for ${idString} (will be reconstructed at runtime)`);
        }

        const jsContent = generateJSContent(idString, base64Data, 'png', pngFile, positioningData);

        fs.writeFileSync(jsFilePath, jsContent);
        const formatMsg = positioningData ? '(with positioning data - legacy)' : '(no positioning)';
