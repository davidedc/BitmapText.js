...
// Function to generate JS content for image data with optional positioning
function generateJSContent(idString, base64Data, imageType, originalFilename, positioningData = null) {
...
if (typeof AtlasExpander !== 'undefined' && typeof FontLoader !== 'undefined') {
  ...
  FontLoader._tempAtlasPositioning['${idString}'] = ${JSON.stringify(positioningData)};
}
`;
    }
...
// MIME type: ${mimeType}
// Encoding: base64
${positioningData ? '// Contains positioning data from corresponding atlas file' : '// No positioning data available - will use simplified placeholders'}
${positioningCode}
// Register ${imageType.toUpperCase()} image data
if (typeof FontLoader !== 'undefined' && FontLoader.registerTempAtlasData) {
    FontLoader.registerTempAtlasData('${idString}', '${base64Data}');
} else {
    console.warn('FontLoader not available - atlas data for ${idString} not registered');
}
`;
}
...
if (processPNG) {
    ...
    pngFiles.forEach(pngFile => {
        ...
        // Try to read corresponding positioning data
        ...
        log(`Processed PNG: ${pngFile} -> ${jsFileName}${positioningData ? ' (with positioning data)' : ''}`);
...
if (processQOI) {
    ...
    qoiFiles.forEach(qoiFile => {
        ...
        fs.writeFileSync(jsFilePath, jsContent);
        log(`Processed QOI: ${qoiFile} -> ${jsFileName}${positioningData ? ' (with positioning data)' : ''}`);
