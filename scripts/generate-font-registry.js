...
function generateFontRegistry() {
  ...
  if (!fs.existsSync(fontAssetsDir)) {
    console.error('❌ Error: font-assets directory not found');
  ...
  try {
    ...
    console.error(`❌ Error reading font-assets directory: ${error.message}`);
    process.exit(1);
  }
  ...
  if (fontIDs.length === 0) {
    console.warn('⚠️  Warning: No metrics files found in font-assets/');
...
if (typeof FontManifest !== 'undefined') {
  ...
  try {
    ...
    console.error(`❌ Error writing font-registry.js: ${error.message}`);
    process.exit(1);
  }

  // Success message
  log(`✅ Generated font-registry.js with ${fontIDs.length} font ID${fontIDs.length === 1 ? '' : 's'}`);
  ...
  log(`\n💡 Usage: Include font-registry.js in test-renderer.html to load all available fonts`);
}
