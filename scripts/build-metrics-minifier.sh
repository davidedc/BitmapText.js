...
# Concatenate StatusCode (required by BitmapText.js)
echo "// STATUS CODE CONSTANTS" >> "$OUTPUT_FILE"
echo "// Required by BitmapText.js (dependency check at class definition)" >> "$OUTPUT_FILE"
echo "// ============================================================================" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
...
function processFullMetricsFiles() {
  ...
  if (!fs.existsSync(fontAssetsDir)) {
    console.error('❌ font-assets/ directory not found');
  ...
  if (files.length === 0) {
    console.log('\n⚠️  No *-full.js files found in font-assets/');
    console.log('');
  ...
  // Print summary
  console.log('═'.repeat(60));
  console.log(`📊 Summary: ${successCount} successful, ${errorCount} error(s)`);
  console.log('═'.repeat(60));
  ...
  if (successCount > 0) {
    ...
    successResults.forEach(r => {
      ...
      console.log(`     ${r.originalSize.toLocaleString()} → ${r.minifiedSize.toLocaleString()} chars (${r.reduction}% reduction)`);
    });
